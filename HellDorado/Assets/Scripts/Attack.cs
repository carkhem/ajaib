	...
	public void Strike(){
		...
		if (Physics.Raycast (transform.position + offset, transform.TransformDirection (Vector3.forward), out hit, reach)) {
            ...
            if (hit.transform.CompareTag("Enemy")) {
                ...
                hit.transform.GetComponentInParent<HitParticle>().SpawnHitParticle(hit.transform);
                hit.transform.GetComponentInParent<EnemySounds>().PlayHitSound();
			}

            if (hit.transform.CompareTag("Player")){
				PlayerStats.instance.ChangeHealth (-20);
				Camera.main.GetComponent<Animator> ().SetTrigger ("hit");
                hit.transform.GetComponent<PlayerSounds>().PlayHitSound();
